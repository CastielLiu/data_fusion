...
{
  invRt = cameraExtrinsicMat(cv::Rect(0, 0, 3, 3)); //旋转矩阵，左上角3x3
  
  //.t()转置矩阵
  cv::Mat invT = -invRt.t() * (cameraExtrinsicMat(cv::Rect(3, 0, 1, 3))); //平移矩阵
  invTt = invT.t();
  init_matrix = true;
}
						  ...
						  const cv::Mat& distCoeff, const cv::Size& imageSize)
{
  ...
  std::vector<float> distance(w*h,0);
  ...
  for (uint32_t y = 0; y < pointcloud2->height; ++y)
  {
    ...
    {
      float* fp = (float*)(cp + (x + y * pointcloud2->width) * pointcloud2->point_step); //point ptr
      double intensity = fp[4]; //x,y,z,intensity
      ...
      if (0 <= px && px < w && 0 <= py && py < h)
      {
        ...
        int pid = py * w + px;
        //考虑多个雷达点投影在同一像素，距离近的点覆盖距离远的点
        if (distance[pid] == 0 || distance[pid] > point.at<double>(2))
        {
           distance[pid] = point.at<double>(2);
           if(max_distance < distance[pid] )
                max_distance = distance[pid];
           
            //反射率填充像素值
            //距离填充像素值
            //最后使用最大值归一化像素
            
           image.at<cv::Vec3b>(py,px) = (0,0,0);
