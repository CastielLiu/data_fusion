...
void ROSPixelCloudFusionApp::ImageCallback(const sensor_msgs::Image::ConstPtr &in_image_msg)
{
	...
	if(is_raw_image_)
		cv::undistort(cv_image->image, current_frame_, camera_instrinsics_, distortion_coefficients_);
	else
...
void ROSPixelCloudFusionApp::InitializeROSIo(ros::NodeHandle &nh_private)
{
	//get params
	...
	nh_private.param<std::string>("points_src", points_src, "/points_raw");
	ROS_INFO("[%s] points_src: %s", __APP_NAME__, points_src.c_str());

	nh_private.param<std::string>("image_src", image_src, "/image_rectified");
	ROS_INFO("[%s] image_src: %s", __APP_NAME__, image_src.c_str());

	nh_private.param<std::string>("camera_info_src", camera_info_src, "/camera_info");
