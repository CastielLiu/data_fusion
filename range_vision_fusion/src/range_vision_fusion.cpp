...
cv::Rect ROSRangeVisionFusionApp::ProjectDetectionToRect(const autoware_msgs::DetectedObject &in_detection)
{
  ...
  Eigen::Affine3f range_vision_tf;//仿射变换矩阵4x4
...
//图像检测目标与雷达检测目标融合函数
/*
1. 首先进行雷达监测目标分类(相机视野内、相机视野外)
2. 对相机视野内的雷达目标进行平面投影(投影的图像平面得到矩形边框rect_lidar)
3. 遍历查找与图像目标边框rect_img重合度高的rect_lidar, 
   *1. 可能存在多个rect_lidar与同一个rect_img匹配, 但最终融合的时候只保留最近的目标物，
   *2. 也可能存在
4. 利用匹配关系，将图像目标的关键信息填充到雷达目标(颜色，类别，等)
5. 填充融合结果
   *1. 与图像目标重合的雷达目标，填入融合结果(如多雷达目标匹配同一图像目标，取近者)
   *2. 没有匹配到图像目标的雷达目标，或者匹配到但重复匹配且较远的目标
   *3. 没有匹配到雷达目标的图像目标，填入融合结果
   *4. 图像范围之外的雷达目标，填入融合结果
  ...
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections)
{
  ...
  for (size_t i = 0; i < in_vision_detections->objects.size(); i++)
  {
    ...
    //雷达检测到的物体，在摄像头视野可能存在重叠，因此因选择近处的物体
  ...
  for (size_t i = 0; i < vision_range_assignments.size(); i++)
  {
    ...
    if (vision_range_closest[i] >= 0)
    {
      used_range_detections[vision_range_closest[i]] = true;
      
      //与图像目标重合的雷达目标，填入融合结果(如多雷达目标匹配同一图像目标，取近者)
  ...
  for (size_t i=0; i<range_in_cv.objects.size(); ++i)
  {
    ...
    {
      //没有匹配到图像目标的雷达目标，或者匹配到但重复匹配且较远的目标
